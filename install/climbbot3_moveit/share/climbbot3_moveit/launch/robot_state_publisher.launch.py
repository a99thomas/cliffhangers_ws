...
def generate_launch_description():
    ...
    """
    # Define filenames
    urdf_package = 'climbbot3_moveit'
    urdf_filename = 'climbbot3.urdf.xacro'
    rviz_config_filename = 'moveit.rviz'
    # rviz_config_filename = 'rsp.rviz'

    # Set paths to important files
    pkg_share_description = FindPackageShare(urdf_package)
    pkg_description = FindPackage(urdf_package)

    default_urdf_model_path = PathJoinSubstitution(
        [pkg_share_description, 'config', urdf_filename])
        
    
    default_rviz_config_path = os.path.join(
        str(Path.home()),
        'workspaces/cliffhangers_ws/src/climbbot3_moveit/config/moveit.rviz',
    )
    # default_rviz_config_path = PathJoinSubstitution(
    #     [pkg_description, 'config', rviz_config_filename])
    ...
    # Resolve the path by performing the substitution
    # resolved_path = default_urdf_model_path.perform(context)
    def print_urdf_path(context):
        resolved_path = default_urdf_model_path.perform(context)
        print("Resolved URDF Path:", resolved_path)
        return []
    def print_urdf_contents(context):
        # Resolve the URDF file path
        resolved_path = default_urdf_model_path.perform(context)

        # Prepare the xacro command
        xacro_cmd = ['xacro', resolved_path]

        try:
            # Run the xacro command and capture its output
            result = subprocess.run(xacro_cmd, capture_output=True, text=True, check=True)
            urdf_contents = result.stdout
            print("URDF Model Contents:\n", urdf_contents)
