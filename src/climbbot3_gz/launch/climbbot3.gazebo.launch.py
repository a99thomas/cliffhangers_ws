...
def generate_launch_description():
    ...
    package_name_gazebo = 'climbbot3_gz'
    package_name_description = 'climbbot3_description'
    package_name_moveit = 'climbbot3_moveit'

    default_robot_name = 'climbbot3'
    ...
    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(pkg_share_moveit, 'launch', 'robot_state_publisher.launch.py')
        ]),
        launch_arguments={
            'jsp_gui': jsp_gui,
    ...
    load_controllers_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(pkg_share_moveit, 'launch', 'spawn_gz_controllers.launch.py')
        ]),
        launch_arguments={
            'use_sim_time': use_sim_time
        }.items(),
    ...
    spawn_wall_cmd = Node(
            ...
            output='screen',
            arguments=[
                '-file',  "/home/aaron/workspaces/cliffhangers_ws/src/climbbot3_gz/models/climbing_wall/robot.urdf"
            ])

    camera_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(pkg_share_moveit, 'launch', 'camera_state_publisher.launch.py')
        ]),
        launch_arguments={
            'use_sim_time': use_sim_time
        }.items(),
    ...
    spawn_camera_cmd = Node(
        ...
        executable='create',
        output='screen',
        arguments=['-string', processed_camera_urdf]
    )
    ...
    start_move_group_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
    ...
    start_fake_odom_cmd = Node(
        package='climbbot3_moveit',
        executable='gz_odom_tf.py',
        output='screen'
    )
    ...
    start_camera_tf_cmd = Node(
        ...
        arguments=[
                '0.22', '-0.29', '0.5',     # x, y, z
                '1.571', '0.436', '0',        # roll, pitch, yaw (in radians)
                'odom', 'camera_head_link' # parent and child frame
            ],)
    ...
    # ld.add_action(camera_state_publisher_cmd)
    ...
    ld.add_action(spawn_wall_cmd)
    # ld.add_action(spawn_camera_cmd)
    ld.add_action(start_fake_odom_cmd)
